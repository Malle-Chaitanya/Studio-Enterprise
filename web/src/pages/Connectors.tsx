  ...
  return (
    <div className="card wide">
      ...
      <div className="wizard-actions" style={{ marginTop: 20 }}>
        <button className="wbtn" onClick={() => navigate(`/map?session=${session}`)}>← Back</button>
        <button className="wbtn primary" onClick={() => navigate(`/select-data?session=${session}`)}>Continue →</button>
      </div>
    </div>
  );
}
