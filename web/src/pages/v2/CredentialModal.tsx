...
const VALIDATION_TEXT: Record<ConnectorValidation['code'], string> = {
  ok: 'Tested against the provider — the credential works.',
...
export function CredentialModal({
  ...
  const outstanding = fields.filter((f) => !f.supplied);
            ...
            if (f.supplied) {
              return (
                <div className={`v2-fld supplied${secret ? ' secret' : ''}`} key={f.key}>
                  <label htmlFor={`f-${f.key}`}>{f.label} <em>— already stored</em></label>
                  <input
                    id={`f-${f.key}`}
                    className="v2-field"
                  ...
                  <span className="byagent">
                    Reused from your earlier setup. The value is never read back into this page.
                  </span>
                </div>
              );
            }
