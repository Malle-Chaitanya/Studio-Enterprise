...
function requestAgent(input) {
  ...
  // DIAGNOSTIC: log the real HTTP status and a body preview before parsing anything.
  console.log(`HTTP status: ${response.getResponseCode()}`);
  const responseContentText = response.getContentText();
  console.log(`Raw body (first 800 chars): ${responseContentText.slice(0, 800)}`);

  // Process the SSE response (one line per event)
  const events = responseContentText
  .split(/\n\n+/)
  .map(block => block
    .split('\n')
