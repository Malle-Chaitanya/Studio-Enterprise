...
async function main() {
  ...
  const agentId = '5654538465772525334'; // Email Manager (Outlook)

  const res = await fetch(`${base}/agents/${agentId}:getIamPolicy`, { method: 'GET', headers: { Authorization: `Bearer ${token}` } });
