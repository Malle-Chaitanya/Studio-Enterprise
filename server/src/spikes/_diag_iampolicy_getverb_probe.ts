...
async function main() {
  ...
  const createRes = await fetch(AGENT_BASE, {
    ...
    body: JSON.stringify({
      displayName: 'ZZ-iampolicy-getverb-probe',
      description: 'throwaway - delete immediately',
      lowCodeAgentDefinition: {
        rootAgentId: 'root_agent',
        nodes: [{ id: 'root_agent', displayName: 'ZZ-iampolicy-getverb-probe', llmAgentNode: { description: 'test', model: 'gemini-2.5-flash', instruction: 'Say hello.', subAgentIds: [] } }],
      },
    }),
  });
  ...
  const res = await fetch(url, { method: 'GET', headers: { Authorization: `Bearer ${saToken}` } });
  console.log('GET :getIamPolicy ->', res.status, (await res.text()).slice(0, 400).replace(/\n/g, ' '));
