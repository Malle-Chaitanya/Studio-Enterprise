...
async function main() {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      session: null,
      query: { text: 'List the files in my Google Drive root folder' },
      agentsSpec: { agentSpecs: [{ agentId: AGENT_ID }] },
    }),
  });
  console.log('HTTP status:', res.status);
  const text = await res.text();
  writeFileSync('C:/Users/CHAITA~1/AppData/Local/Temp/claude/C--Users-ChaitanyaMalle-Studio-Enterprise-Studio-Enterprise/33f35a95-8f9f-4dcf-ab87-9f2d26f4aa42/scratchpad/drive_test.json', text);
