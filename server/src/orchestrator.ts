  ...
  await mapPool(staged, INSERT_CONCURRENCY, async (row) => {
    ...
    try {
      ...
      const create: CreateOutcome = await (async () => {
            ...
            // Combine every resolved grounding source — files, Dataverse-
            // snapshot, SharePoint-connector — into one list. adk_deploy.py
            // (2026-08-05 fix, live-verified against 2 real combined stores)
            // wires a single store via the built-in VertexAiSearchTool, and
            // 2+ stores via hand-rolled, distinctly-named FunctionTools
            // instead of combining VertexAiSearchTool instances (which
            // crashed every query — see decisions.md for the full history:
            // data_store_specs misuse → missing runtime dependency →
